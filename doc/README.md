...
| [05-工具系统.md](05-工具系统.md) | Tool 接口与风险分级、策略门与审批、内置工具清单、functools 纯函数工具 |
| [06-Skill与MCP.md](06-Skill与MCP.md) | Skill 约定与注入、MCP stdio 客户端与工具桥接 |
| 能力接入契约（[01-总体架构](01-总体架构.md)） | `internal/capability/`：Preload / Tools / Capture 三通道，Registry 串联装配 |
| [07-聊天与会话.md](07-聊天与会话.md) | 流式主链路、SSE 事件契约与可靠性、审批/补充输入、续跑 |
| [08-记忆系统.md](08-记忆系统.md) | 情景/语义/程序记忆、形成打分与召回 |
| [09-知识库与RAG.md](09-知识库与RAG.md) | Loader / Chunker / 索引 / 检索 |
| [10-工作流引擎.md](10-工作流引擎.md) | 图模型、节点类型、执行器、暂停与恢复 |
| [11-定时任务与通知通道.md](11-定时任务与通知通道.md) | cron 调度、邮件 / Webhook / 控制台通道 |
| [12-桌宠.md](12-桌宠.md) | 形态切换、状态机、sprite 播种、迷你聊天、托盘 |
| [13-前端架构.md](13-前端架构.md) | 双主机前端、状态管理、流式渲染、设计 token、聊天交互 |
| [14-构建与分发.md](14-构建与分发.md) | 开发/验收命令、wails 构建、NSIS、架构门禁 |
| [15-开发流程与命令.md](15-开发流程与命令.md) | 初始化、日常开发、单独验证、调试要点、提交与分支 |
| [16-API契约总表.md](16-API契约总表.md) | 全部路由与字段规范 |
| [17-内置运行时.md](17-内置运行时.md) | Node / Python / PowerShell 解压与 PATH 注入 |
...
| 记忆 | 长期 / 情景 / 语义 / 程序 多层；确定性打分形成，召回不阻塞首 token（短期上下文由 chat.toLLMMessages 负责） |
| 知识库 | 多格式加载、分块索引、检索（FTS5 trigram + 短词 LIKE 兜底） |
...
| 可观测 | run 事件 JSONL 导出（回放/测试/无头消费）；run 级索引落 SQLite（run_records），三通道日志 |
